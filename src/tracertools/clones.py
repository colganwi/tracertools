...
def pairwise_combs(clones):
    ...
    return dict(zip(key_combs, list_combs, strict=False))
...
def plot_whitelist_alleles(alleles, top_n=100, plot_title=None):
    ...
    if "UMI" in colored_ints.columns:
        ...
        colored_ints = pd.pivot_table(
            colored_ints, index=["clone", "cellBC"], columns="intID", values="color", aggfunc="sum"
        ).fillna(0)
    else:
        colored_ints["color"] = (colored_ints["whitelist"] * 2 - 1) * np.log10(colored_ints["intBC_intensity"])
        colored_ints = pd.pivot_table(
            colored_ints, index=["clone", "cellBC"], columns="intID", values="color", aggfunc="max"
        ).fillna(0)
    ...
    sns.clustermap(
        colored_ints, figsize=(6, 6), vmax=vmax, vmin=-vmax, cmap=cmap, cbar_pos=None, dendrogram_ratio=(0, 0)
    )
    plt.title(plot_title)


def call_clones(
    alleles, model=None, min_frac=0.5, min_jaccard=0.5, doublets=True, top_n=None, plot=True, plot_title=None
):
    """Call clones based on set of integrations."""
    # Identify clones using clustering algorithm
    int_counts = pd.pivot_table(
        alleles.assign(present=1), index=["cellBC"], columns="intID", values="present", aggfunc="first"
    ).fillna(0)
    ...
    cell_to_clone["clone"] = (
        cell_to_clone["intID"].apply(lambda x: max_jaccard(x, clone_ints, min_jaccard=min_jaccard)).astype(str)
    )
...
def assign_clones(
    alleles, clone_whitelist, min_jaccard=0.5, doublets=True, fill=-1, top_n=100, plot=True, plot_title=None
):
    ...
    cell_to_clone["clone"] = (
        cell_to_clone["alleles"]
        .apply(max_jaccard, groups=clone_alleles, fill=fill, min_jaccard=min_jaccard)
        .astype(str)
    )
    cell_to_clone["whitelist"] = (~cell_to_clone["clone"].str.contains(",")) & (cell_to_clone["clone"] != "-1")
    # Whitelist alleles
