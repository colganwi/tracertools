...
def get_parser():
    ...
    parser.add_argument(
        "--site_positions",
        type=str,
        default="{'RNF2':332,'HEK3':380,'EMX1':448}",
        help="Dictionary mapping site names to positions",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Extract barcode sequence from reads instead of using alignment",
    )
...
def call_alleles_10x(param):
    ...
    for read in read_iter:
        if (
            ...
            or read.reference_start > barcode_start
            or read.reference_end < end
            or "N" in read.cigarstring
        ...
        # Get integration
        if extract_barcode:
            intID = barcode_from_alignment(
                read.query_sequence, read.cigarstring, barcode_start, barcode_end, read.reference_start
            )
        else:
    ...
    if len(umi_counts) > 0:
        ...
        # correct UMIs
        allele_counts = (
            allele_counts.groupby(["intID", "cellBC", "UMI"] + site_names)
            .agg({"readCount": "sum"})
        ...
        # collapse UMIs
        allele_counts = (
            allele_counts.groupby(["intID", "cellBC"] + site_names)
            .agg({"UMI": "size", "readCount": "sum"})
            .reset_index()
        )
        # filter alleles
...
def alleles_from_bam(
    bam, out, barcode_start, barcode_end, site_positions, min_reads=10, extract_barcode=False, **kwargs
):
    # Parse the arguments
    ...
    if has_cb:
        ...
        if extract_barcode:
            call_alleles_10x(
                (intIDs[0], bamfile, barcode_start, barcode_end, sites, min_reads, extract_barcode, out, lock)
            )
        else:
